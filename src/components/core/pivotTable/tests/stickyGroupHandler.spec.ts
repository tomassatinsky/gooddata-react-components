...
describe('updateStickyHeaders', () => {
    ...
    it('should hide the sticky row when the row data are not available (still loading)', () => {
        ...
        expect(fakeGridApiWrapper.addPinnedTopRowClass).toHaveBeenCalledWith(fakeGridApi, 'gd-hidden-sticky-row');
        assertOnlyListedMethodsHaveBeenCalled(fakeGridApiWrapper, ['addPinnedTopRowClass']);
    });
    ...
    describe('current scroll position belongs to different line than the last one', () => {
        ...
        it('should show the sticky row', () => {
            expect(fakeGridApiWrapper.removePinnedTopRowClass)
                .toHaveBeenCalledWith(fakeGridApi, 'gd-hidden-sticky-row');
        });
